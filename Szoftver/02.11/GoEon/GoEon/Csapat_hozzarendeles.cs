    ...
    public partial class Csapat_hozzarendeles : Form
    {
        ...
        private void feladatok_listBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (feladatok_listBox.SelectedItem != null)
            {
                ...
                {
                    if (fe.getFeladat_neve().Equals(feladatok_listBox.SelectedItem.ToString()))
                    {
                        StringBuilder sb = new StringBuilder();
                        sb.Append("www.google.com/maps/place/");
                        sb.Append(fe.getVaros() + ",+" + fe.getUtca() + "+" + fe.getHazszam());
                        Terkep form = new Terkep(sb.ToString());
                        form.Show();
                        information_listBox.Items.Add(form.getUrl());
                    }
                }
            }
        }
