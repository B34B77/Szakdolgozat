...
namespace GoEon
{
    public partial class Uj_feladat_felvetele : Form
    {
        ...
        public Uj_feladat_felvetele()
        {
            InitializeComponent();

            equipments = DataManager.BeolvasEszkozok(equipments);
            persons = DataManager.BeolvasKEmberek(persons);
        }

        private void Uj_feladat_felvetele_Load(object sender, EventArgs e)
        {
            //Határidő dateTimePicker beállítása a kívánt formátumra
            hatarido.Text = DateTime.Now.Year + "-" + DateTime.Now.Month + "-" + DateTime.Now.Day;
        ...
        private void vissza_button_Click(object sender, EventArgs e)
        {
            ...
            fooldal.Closed += (s, args) => this.Close();
            fooldal.Show();
        }
        ...
        private void feladat_felvetele_button_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                cmd.Connection = conn;
                cmd.CommandText = "INSERT INTO task_table(task_id, task_name, city, street, house_number, description, deadline, recording_date, priority, equipments, qualification, comment, working_hour, done) VALUES(null, @task_name, @city, @street, @house_number, @description, @deadline, now(), @priority, @equipments, @qualification, @comment, @working_hour, @done)";
                cmd.Prepare();
                ...
                if (good)
                {
                    ...
                    cmd.Parameters.AddWithValue("@done", "-");
