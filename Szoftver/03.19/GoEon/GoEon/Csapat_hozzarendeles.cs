...
namespace GoEon
{
    public partial class Csapat_hozzarendeles : Form
    {
        ...
        public Csapat_hozzarendeles()
        {
            administrations = DataManager.BeolvasAdministration(administrations);
            osszes_feladat = DataManager.BeolvasFeladatok(osszes_feladat);
        ...
        public Csapat_hozzarendeles(string template)
        {
            administrations = DataManager.BeolvasAdministration(administrations);
            osszes_feladat = DataManager.BeolvasFeladatok(osszes_feladat);
            csapatok = DataManager.getTeamList();
        ...
        public void load_feladatok_listBox()
        {
            ...
            //Kitörli a feladatok közül azt amelyik már hozzá lett rendelve egy csapathoz (ideiglenesen)
                        ...
                        if (team_listBox.SelectedItem.Equals(team.getTeam_name()))
                        {
                            ...
                            else
                            {
                                foreach (Feladatok fe in feladatok)
                                {
                                    ...
                                    {
                                        int compare = String.Compare(dateTimePicker.Text, fe.getHatarido());
                                        ...
                                        {
                                            DialogResult hiba = MessageBox.Show("A megadott teljesítés dátuma késöbbi, mint a feladat határideje!\n", "Hiba", MessageBoxButtons.OK);
                                            good = false;
                                        }
            ...
            {                             
                foreach(Feladatok fe in feladatok)
                {
                    if (fe.getFeladat_neve().Equals(tasks_listBox.SelectedItem))
                    {
                        ...
                        {
                            DialogResult warning = MessageBox.Show("A megadott teljesítés dátuma késöbbi, mint a feladat határideje!\n", "Hiba", MessageBoxButtons.OK);
